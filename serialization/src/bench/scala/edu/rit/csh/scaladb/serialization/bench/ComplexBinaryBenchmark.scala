...
class ComplexBinaryBenchmark extends Bench.OfflineReport with PrimitiveGenerators {
  ...
  val strList = length.map(len => (0 to len).map(_ => "this is a test string, who knows what could happen here").toList)
  ...
  performance of "Complex Serialization" in {
    measure method "old string lists" in { using(strList) in { l => l.oldBinary() } }
    measure method "new string lists" in { using(strList) in { l => l.binary() } }
    measure method "old int arrays" in { using(intArray) in { a => a.oldBinary() } }
    measure method "new int arrays" in { using(intArray) in { a => a.binary() } }
    measure method "old int lists" in { using(intLists) in { l => l.oldBinary() } }
